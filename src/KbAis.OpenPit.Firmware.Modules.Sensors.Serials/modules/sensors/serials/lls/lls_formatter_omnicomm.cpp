#include "lls_formatter_omnicomm.h"
...
lls_formatter_omnicomm::DecodeResult_t
lls_formatter_omnicomm::DecodeFirstReply(const QByteArray& bytes) {
    ...
    return std::make_pair(scanned + 6, lls_reply_readdata {
        llsAdr
...
QByteArray
lls_formatter_omnicomm::EncodeOpSingleRead(const std::vector<quint8>& addresses) {
...
lls_formatter_omnicomm::DecodeResult_t
lls_formatter_omnicomm::ReturnUnexpected(int scaned, DecodeReplyError error) {
